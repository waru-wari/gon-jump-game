 ...
 */
import { hmacVerify, hmacSign } from './crypto.js';
 ...
 * @returns {Promise<{ ok: boolean, status?: number, error?: string }>}
 */
export async function verifyScoreSig(score, level, sessionToken, sig, masterSecret) {
  // Re-derive the per-session secret the same way game-start.js did:
  //   sessionSecret = hmacSign(sessionToken, masterSecret)
  // Then verify the client's signature against the derived secret.
  const sessionSecret = await hmacSign(sessionToken, masterSecret);
  const data = `${score}|${level}|${sessionToken}`;
  const valid = await hmacVerify(data, sig, sessionSecret);
  if (!valid) {
